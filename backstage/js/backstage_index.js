...
$(".edit").click(function (e) {
  ...
  if (editBtn.innerText == "編輯") {
    ...
    td.children(".editValue")
      .td.children(".QUIZ_CON")
      .replaceWith(
        `<textarea class="${quizClass}" cols="20" rows="5">${quizCon}</textarea>`
      );
    td.children(".QUIZ_ONE_CONTENT").replaceWith(
      `<textarea class="${QUIZ_ONE_CONTENTclass}" cols="20" rows="5">${quizOneContent}</textarea>`
    );
    td.children(".QUIZ_TWO_CONTENT").replaceWith(
      `<textarea class="${QUIZ_TWO_CONTENTclass}" cols="20" rows="5">${quizTwoContent}</textarea>`
    );

    //career
    td.children(".indName").replaceWith(
      `<input type="text" class="${indNameClass}" value="${indName}">`
    );
    td.children(".indIntro").replaceWith(
      `<textarea class="${indIntroClass}" cols="20" rows="5">${indIntro}</textarea>`
    );
    td.children(".indContent").replaceWith(
      `<textarea class="${indContentClass}" cols="20" rows="5">${indContent}</textarea>`
    );
    td.children(".indSkill").replaceWith(
      `<textarea class="${indSkillClass}" cols="20" rows="5">${indSkill}</textarea>`
    );
    td.children(".firstYearLow").replaceWith(
      `<input type="text" class="${firstYearLowClass}" value="${firstYearLow}">`
    );
    td.children(".firstYearHigh").replaceWith(
      `<input type="text" class="${firstYearHighClass}" value="${firstYearHigh}">`
    );
    td.children(".thirdYearLow").replaceWith(
      `<input type="text" class="${thirdYearLowClass}" value="${thirdYearLow}">`
    );
    td.children(".thirdYearHigh").replaceWith(
      `<input type="text" class="${thirdYearHighClass}" value="${thirdYearHigh}">`
    );
    td.children(".fifthYearLow").replaceWith(
      `<input type="text" class="${fifthYearLowClass}" value="${fifthYearLow}">`
    );
    td.children(".fifthYearHigh").replaceWith(
      `<input type="text" class="${fifthYearHighClass}" value="${fifthYearHigh}">`
    );
    td.children(".tenYearLow").replaceWith(
      `<input type="text" class="${tenYearLowClass}" value="${tenYearLow}">`
    );
    td.children(".tenYearHigh").replaceWith(
      `<input type="text" class="${tenYearHighClass}" value="${tenYearHigh}">`
    );
    td.children(".upYearLow").replaceWith(
      `<input type="text" class="${upYearLowClass}" value="${upYearLow}">`
    );
    td.children(".upYearHigh").replaceWith(
      `<input type="text" class="${upYearHighClass}" value="${upYearHigh}">`
    );

    //quiz換照片
    let QUIZ_PIC_ONE, quizImgOne, QUIZ_PIC_TWO, quizImgTwo, indPic, indImg;
    ...
    $(".cancel").click(function () {
      ...
      td.children(".QUIZ_CON").replaceWith(
        `<div id="${quizClass}" cols="20" rows="5">${quizCon}</div>`
      );
      td.children(".QUIZ_ONE_CONTENT").replaceWith(
        `<div id="${QUIZ_ONE_CONTENTclass}" cols="20" rows="5">${quizOneContent}</div>`
      );
      td.children(".QUIZ_TWO_CONTENT").replaceWith(
        `<div id="${QUIZ_TWO_CONTENTclass}" cols="20" rows="5">${quizTwoContent}</div>`
      );
