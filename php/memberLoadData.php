<?php
    try{
        ...
        if( $pdo != false ){
            ...
            $sqlClassBuy = "select mem.MEM_NAME, skill.SKI_NAME, skill.SKI_IMG, skill.SKI_TEC_NAME from `order_detail` item join `order_mem` ord on item.ORD_NO = ord.ORD_NO join `member` mem on ord.MEM_NO = mem.MEM_NO join `skill_class` skill on item.SKI_NO = skill.SKI_NO where ord.MEM_NO = :memNo";
